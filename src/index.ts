...
async function main() {
  ...
  try {
    ...
    const hamster_stack = await runGetmethod("get_wallet_state_init_and_salt", HMSTRMasterAddress, userAddress);
    ...
    const hamster_root = await runGetmethod("get_mintless_airdrop_hashmap_root", HMSTRMasterAddress, userAddress);
    ...
    if (jettonData && jettonData.walletCode) {
      ...
      const jettonWalletStateInit = calcHMSTRstateinit(userAddress, HMSTRMasterAddress, walletCode, hamster_merkle, hamster_salt)
      const userJettonWalletAddress = new Address(0, (await jettonWalletStateInit).hash())
